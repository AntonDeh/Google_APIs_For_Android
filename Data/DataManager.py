...
class Container:
    ...
    def get_object_lists_differences(self, data, html_doc):
        ...
        differences_html_doc = [item for item in data if item.sha not in extracted_sha
                                and self._is_valid_obj(item)]

        extracted_sha = [x.sha for x in data]
        differences_file = [item for item in html_doc if item.sha not in extracted_sha]

        missing_sw_versions['from_html'] = differences_file
        missing_sw_versions['from_db'] = differences_html_doc
